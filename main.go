...
import (
	...
	"github.com/UniDropOfficial/unibus-go/unibus"
)

func main() {
	client := unibus.New(context.TODO())
	client.EstablishConnection() // Only necessary for RabbitMQ
	event := unibus.NewEvent("rimdesk.product-api", "rimdesk.inventory.create", "rimdesk.product.create")
	...
	if _, err := client.Publish("rimdesk.product-api", jb, &unibus.PublishOptions{
		Args:        nil,
		AutoDelete:  false,
	...
	messageEvents, err := client.Consume("rimdesk.product-api", &unibus.ConsumeOptions{
		AutoAck:      false,
		ConsumerName: "",
